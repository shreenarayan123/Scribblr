  ...
  async function handleUserUpdate() {
    ...
    if (formData.img || formData.name || formData.bio) {
      try {
        ...
        const res = await axios.put(`${backendUrl}/user/${userId}`, {
    ...
    <Modal modal={editModal} setModal={setEditModal}>
      ...
      <div className="flex flex-col gap-6 absolute top-28 z-20  md:left-[30%] sm:left-10 bg-white shadow-2xl p-10">
        ...
        <div className="flex items-center gap-5">
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.files && e.target.files.length > 0) {
                      setimagePreview(URL.createObjectURL(e.target.files[0])); // createObjectURL() creates a URL that can be used to reference the File or Object.)
          ...
          <div className="flex flex-col gap-4">
            <div className="flex gap-4">
              <span onClick={openFile} className="text-blue-500 cursor-pointer">
                Update
              </span>
              <span
          ...
          <div className="flex">
            <div className="w-full ">
              <textarea
                ...
                maxLength={100}
                className="py-3 px-4 block bg-gray-100 w-full border-gray-200 rounded-lg text-sm focus:border-black focus:ring-black disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600"
              ></textarea>
        ...
        <div className="flex items-center justify-end gap-5 mt-3">
          ...
          <button
            disabled={
              !imageUrl && !(formData.name.length && formData.bio.length)
            }
            onClick={handleUserUpdate}
            type="button"
            className={`text-white bg-green-900 border border-gray-200 focus:outline-none  ${
              !imageUrl && !(formData.name.length && formData.bio.length)
                ? "text-blue-300 border-gray-300 bg-blue-100 "
                : ""
            } 
